...
//Shuffle Cards//
var shuffleCards = function (makeDeck) {
  var currentIndex = 0;
  while (currentIndex < makeDeck.length) {
    var randomIndex = randomNumber(makeDeck.length);
    var randomCard = makeDeck[randomIndex];
    var currentCard = makeDeck[currentIndex];
...
var moarCardsSingleCardMain = function (input) {
  ...
  var message = `You drew the ${drawCard.name} of ${drawCard.suit}`;
...
var moarCardsLowCardMain = function (input) {
  ...
  //Draw cards//
  var drawCard1 = shuffledDeck.pop();
  var drawCard2 = shuffledDeck.pop();

  var message = `Player One drew the ${drawCard1.name} of ${drawCard1.suit} while Player Two drew the ${drawCard2.name} of ${drawCard2.suit}.<br><br>`;

  //Find lowest value//
  if (drawCard1.rank < drawCard2.rank) {
    message += `Player One won!`;
  } else if (drawCard1.rank == drawCard2.rank) {
    message += `Both player draw.`;
  } else message += `Player Two won!`;
...
var moarCardsLowCardQueenWinnerMain = function (input) {
  ...
  if (drawCard1.rank == 12 && drawCard2.rank != 12) {
    return (message += "Player One won!");
  } else if (drawCard2.rank == 12 && drawCard1.rank != 12) {
    return (message += `Player Two won!`);
  } else if (drawCard1.rank == 12 && drawCard2.rank == 12) {
    return (message += `Both players draw.`);
  }
...
var moarCardsLowCardHandsMain = function (input) {
  ...
  if (numberOfCards > 0) {
    var computer = shuffledDeck.pop();
    message = `The computer drew the ${computer.name} ${computer.suit}.<br><br>You drew the following cards:<br>`;
  }

  //Draw x number of cards//
  for (var i = 0; i < numberOfCards; i += 1) {
    var drawCards = shuffledDeck.pop();
    message += `${drawCards.name} ${drawCards.suit}<br>`;
    playerCards.push(drawCards);
  }

  //Find lowest card number//
  var lowestCard = playerCards[0];
  for (var j = 0; j < numberOfCards; j += 1) {
    if (playerCards[j].rank < lowestCard.rank) {
      lowestCard = playerCards[j];
    }
  }
  message += `<br>Your lowest card is ${lowestCard.name} ${lowestCard.suit}.<br><br>`;

  //Find queen//
  var queen = false;

  for (var k = 0; k < numberOfCards; k += 1) {
    if (playerCards[k].rank == 12) {
      queen = true;
    }
  }
  ...
  if (computer.rank == 12 && queen == false) {
    return (message += "Computer drew a Queen! Computer won!");
  } else if (queen == true && computer.rank != 12) {
    return (message += `You drew a Queen! Player won!`);
  } else if (computer.rank == 12 && queen == true) {
    return (message += `Both drew a Queen! It's a draw.`);
  }

  //Find lowest value//
  if (computer.rank < lowestCard.rank) {
    message += `Computer won!`;
  } else if (computer.rank == lowestCard.rank) {
    message += `It's a draw.`;
  } else message += `Player won!`;
